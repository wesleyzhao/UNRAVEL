...
# Script name: prep_tifs.sh
# Description: This script processes tif files by adjusting the display range
#              minimum and making the x and y pixel dimensions even if required.
#              It also adjusts the display range of ochann tif files if needed.
#
# Usage:       ./prep_tifs.sh <min_display_range> [samplefolder01 ...]
# Example:
#
# Parameters:  min_display_range - The minimum value of the new display range for 488 tif files
#              samplef_n   - Optional list of sample folders to process, separated by spaces
#
# Requirements: - FIJI/ImageJ must be installed
#               - The script should be run from the experiment folder containing
#                 the ./sample??/488/tifs and ./sample??/ochann/tifs folders
#               - A metadata.sh script must be available in the sample folders to
#                 retrieve metadata information.
#
# Author:      Austen Casey, Daniel Ryskamp Rijsketic, Heifets lab
# Date:        10/09/21, 12/07/21, 07/07/22, 07/18/22
#
  ...
  if [ "$1" == "0" ] && [ $((SizeX%2)) -eq 0 ] && [ $((SizeY%2)) -eq 0 ]; then #if 488 display range min is 0 and SizeX is even and SizeY is even, then
    ...
    if [ ! -f parameters/488_min ] ; then
      ...
      if [ $((SizeX%2)) -eq 0 ] && [ $((SizeY%2)) -eq 0 ]; then #positional args follow macro w/ # delimiter
        $FIJI_PATH --ij2 -macro prep_tifs $PWD/488_original/$first_488_tif#$1#SizeX_even#SizeY_even#488#_Ch1_
      elif [ $((SizeX%2)) -ne 0 ] && [ $((SizeY%2)) -eq 0 ]; then
        $FIJI_PATH --ij2 -macro prep_tifs $PWD/488_original/$first_488_tif#$1#SizeX_odd#SizeY_even#488#_Ch1_
      elif [ $((SizeX%2)) -eq 0 ] && [ $((SizeY%2)) -ne 0 ]; then
        $FIJI_PATH --ij2 -macro prep_tifs $PWD/488_original/$first_488_tif#$1#SizeX_even#SizeY_odd#488#_Ch1_
      elif  [ $((SizeX%2)) -ne 0 ] && [ $((SizeY%2)) -ne 0 ]; then
        $FIJI_PATH --ij2 -macro prep_tifs $PWD/488_original/$first_488_tif#$1#SizeX_odd#SizeY_odd#488#_Ch1_
      fi
